...
## 综合题
### 使用ryu控制器，搭建网络拓扑图如图所示（controller 表示控制器，switch 表示交换机，host表示主机,server表示服务器。要求： 
#### 1．先使 Host1 可以 ping 通 Host2，Host2 也可以 ping 通 Host1。 
#### 2．对流表进行操作，使 Host1 不能 ping 通 Host2，Host2 也不能 ping 通 Host1。

<img src="/images/qimostudy/sdn.png" width="100%">
    答:
    1.启动ryu,进入源码目录ryu/app,执行下面命令：
    #ryu-manager simple_switch_13.py ofctl_rest.py。此命令启动了交换机模块simple_switch_13.py、   
    控制协议ofctl_rest.py。(3分)  
